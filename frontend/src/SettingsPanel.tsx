...
const PROVIDER_HELP: Record<string, string> = {
  ...
  anthropic: 'Calls the Anthropic API directly -- needs ANTHROPIC_API_KEY (or `ant auth login`).',
...
export function SettingsPanel({
  ...
  // object itself, e.g. after a save) changes underneath us.
  useEffect(() => {
  ...
  const fetchModels = async () => {
    ...
    setModelsError(null)
    try {
      const { models } = await api.llmModels(provider, provider === 'lmstudio' ? baseUrl.trim() || undefined : undefined)
    ...
    >
      ...
      {!settings ? (
        ...
        <>
          ...
          {provider === 'lmstudio' && (
            <label style={{ display: 'block', fontSize: 12, marginBottom: 8 }}>
              <div style={{ color: '#6b7280', marginBottom: 2 }}>Base URL (address:port)</div>
              <input
                value={baseUrl}
                onChange={(e) => setBaseUrl(e.target.value)}
                onBlur={() => {
                  const trimmed = baseUrl.trim()
                  if (trimmed && trimmed !== (providerSettings.base_url ?? '')) {
                    update({ settings: { [provider]: { base_url: trimmed } } })
                  }
                }}
